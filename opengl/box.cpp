  ...
  : x(x), y(y), w(w), h(h)
{
    ...
    }
    {
        ...
        buffer_binding.vertex_attrib_pointer(binding, ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 2, reinterpret_cast<void *>(0 * sizeof(GLfloat)));
...
void Box::draw(glm::vec4 color) const {
    const GLfloat data[] = {
        x,     y,     x + w, y,
        x + w, y,     x + w, y + h,
        x + w, y + h, x,     y + h,
...
void Box::draw_filled(glm::vec4 color) const {
    const GLfloat data[] = {
        x, y, x + w, y,     x + w, y + h,
        x, y, x,     y + h, x + w, y + h
    };
